module.exports = async function(Stable_Version) {
    ...
    const { body } = await got('https://raw.githubusercontent.com/ARIF-VAU-TRA233/A4IFB0T/main/package.json');
    ...
    const LocalVersion = require('../../package.json').version;
        if (Number(LocalVersion.replace(/\./g,"")) < Number(json.version.replace(/\./g,"")) && global.Fca.Require.FastConfig.Stable_Version.Accept == false || Stable_Version && Number(LocalVersion.replace(/\./g,"")) != Number(Stable_Version.replace(/\./g,""))) {
            ...
            log.warn("[ TRA-UPDATE ] •","New Version, Ready to Update: " + LocalVersion + " -> " + Version);    
            await new Promise(resolve => setTimeout(resolve, 3000));
            try {
                execSync(`npm install fca-horizon-remastered@${Version}`, { stdio: 'inherit' });
                log.info("[ TRA-UPDATE ] •","Update Complete, Restarting...");
                await new Promise(resolve => setTimeout(resolve, 3000));
                ...
                try {
                    console.log(err);  
                    log.warn("[ TRA-UPDATE ] •","Update Failed, Trying Another Method 1...");
                    await new Promise(resolve => setTimeout(resolve, 3000));
                    ...
                    try {
                        console.log(err);
                        log.warn("[ TRA-UPDATE ] •","Update Failed, Trying to clean Database() cache...");
                        await new Promise(resolve => setTimeout(resolve, 3000));
                        execSync('npm cache clean --force', { stdio: 'inherit' });
                        log.info("[ TRA-UPDATE ] •","Cache Cleaned, Trying Another Method 2...");
                        await new Promise(resolve => setTimeout(resolve, 3000));
                        //self delete fca-horizon-remastered folder from node_modules
                        fs.rmdirSync((process.cwd() + "/node_modules/tra-arifvauchat-api" || __dirname + '../../../tra-arifvauchat-api'), { recursive: true });
                        await new Promise(resolve => setTimeout(resolve, 3000));
                        execSync(`npm install fca-horizon-remastered@${Version}`, { stdio: 'inherit' });
                        log.info("[ TRA-UPDATE ] •","Update Complete, Restarting...");
                        await new Promise(resolve => setTimeout(resolve, 3000));
                        ...
                        console.log(e);
                        log.error("[ TRA-UPDATE ] •","Update Failed, Please Update Manually");
                        await new Promise(resolve => setTimeout(resolve, 3000));
                        log.warn("[ TRA-UPDATE ] •","Please contact to owner(Mohammad ARIF) about update failed and screentshot error log at https://www.facebook.com/THE.R3B3L.ARIF.VAU/");
                        await new Promise(resolve => setTimeout(resolve, 3000));
                        process.exit(1);
                    }
                }
